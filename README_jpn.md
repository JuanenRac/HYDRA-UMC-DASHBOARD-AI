...
### 主な機能：
* 🧠 **スマートサマリー（v0）** — HYDRA-UMC-DATALAKE の実際の履歴データから計算された、実際の最小値/最大値/平均値/最新値/傾向の統計。最小値/最大値/最新値はそれぞれ、それを生み出した実際の観測タイムスタンプにひも付けられている。*（実際の統計値として実装済み、AI 生成のサマリーはまだ——下記の「ビルドと実行」を参照）*
